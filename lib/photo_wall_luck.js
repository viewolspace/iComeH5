...
$(document).ready(function(){
    ...
    layui.use(['form'], function(){
        ...
        form.on('submit(demo1)', function(data){
            startLuck();//动画加速
            // alert(data.field.rewardLevel);
            var rewardNum = data.field.rewardNum;

            setTimeout(function(){
                endLuck();//取消加速
                showLuck(rewardNum);//展示中奖人员
            }, 7000);


            setTimeout(function(){
                hideLuck();//将中奖人头像飞入中奖名单
            }, 3000);
        ...
        window.showLuck = function (rewardNum) {
            $("#luckDiv").css('display','block');
            $("#luckDiv").css('width',(rewardNum*100)+'px');

        }

        window.hideLuck = function () {
