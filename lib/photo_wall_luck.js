...
showLuck = function (luckLevel, rewardNum, luckList) {
    ...
    if(rewardNum <= 5){
    ...
    }else if(rewardNum>5 && rewardNum <= 10){
        $("#luckDiv").empty();//每次重新渲染中奖名单前，先清空旧数据
        $.each(luckList, function(i,val){  //遍历二维数组
            $("#luckDiv").append('<div class="single"><div class="headpic" ><img src="'+val.url+'" id="luckUserImg" /></div><div class="name">'+val.userName+'</div></div>');
        })
        $("#luckDiv").show();
        $("#luckDiv").css('width',(5*100)+'px');
        ...
        if(rewardNum % 10 ==0){
            ...
            forNum = rewardNum/10 + 1;
        }
        for(var i = 0 ; i < forNum; i++){
            $("#luckDiv").empty();//每次重新渲染中奖名单前，先清空旧数据
            for(var m = i*10; m < (i+1)*10; m++){
                var value = luckList[m];
                $("#luckDiv").append('<div class="single"><div class="headpic" ><img src="'+value.url+'" id="luckUserImg" /></div><div class="name">'+value.userName+'</div></div>');
            }
            $("#luckDiv").show();
            $("#luckDiv").css('width',(5*100)+'px');
